...
2. **标记可疑点**，只标本 skill 强项类。**一致性启发式优先**：中英逐词对应的搭配（强风 strong wind）有一致性促进、风险低；中英不对应的（浓茶正确说法是 strong tea 而非直译 dense tea，强降雨是 heavy rain 而非 strong rain）是负迁移高发区，优先标记（Min et al., 2023; Wang, 2011——83.7% 的虚化动词产出直接镜像中文结构）
   ...
   - L1 迁移高发区（中文背景学习者，用户明示或错误模式可推断时，巡逻优先扫）：冠词与复数遗漏（中文无屈折与冠词系统）、时态不一致、介词选择、run-on 与状语错位（topic-prominent 结构直译）、连接词过度使用与关联连词错配（L1 语篇模式迁移）——依据 5,703 篇 12 学期纵向语料：misformation 51.55%（时态变位/主谓一致/**介词选择**）+ omission 21.30%（冠词/复数/关系代词）为中国学生两大错误类（Sasi & Lai, 2021; Chan, 2010; Li, 2021）；语法类错误仍按路由直接提示，词汇类照常走自查邀请
   - 介词、冠词、词形问题
   - 名词化结构混乱（两个标准框架杂交，如 the modulation of A on B——modulation of B by A 与 effect of A on B 各取一半；判断法：名词化链倒读，问被作用者与作用者位置是否摆反；证据走三路：OpenAlex 对照正确结构 + Ngram 真短语查杂交结构是否近零 + 同义词标准结构频率。实测锚点：modulation of attention on 图书语料近零，effect of attention on 2 万篇、modulation of performance by 3,173 篇成立，真实文本 002，2026-09-20）
...
4. **选择反馈模式**（WCF 文献驱动）：
   ...
   - **确认模式**：四链巡逻 + 查证后零错误标记（或仅剩风格级可选项）时切换。**输出是人话点评，不是盖章报告**——查证过程的存在感降到最低，用户要的是「这文章行不行」的判断和几个有据的观察，不是查证流水账。结构极简：①一两句总评，像懂行的朋友读完说的话；②挑 2-3 个真实查证过的亮点自然地说（不罗列全部验证点，精选纪律同样适用）；③可选项用商量的语气提（「有一处可以商量」而非「优化点 1」）；④下次自查照常，一句。**确认模式禁令**：不用 ✅❌⚠️ 等符号（没有错误就没有标记）；不暴露巡逻链/语步/四链等内部概念（那是 AI 的组织逻辑）；不写「已验证 N 处表达」式的机器账单；不加粗轰炸（好文本的确认不需要视觉强调）；不趁机说教（「查证的价值在于……」这类话删掉）。**人话版式不豁免链接纪律**：自然语气与证据链接并行不悖——数字证据的出处链接行内嵌在句子里（如「revealed 130 万篇级）」），与 v1.5.0 验收标准同样适用，发送前逐行自检（真实文本 002 的实测教训：确认模式最容易把「说人话」误执行成「放弃链接」，两分钟内连续违反两次不同批次的纪律）全绿是合法结论，坦率说「这篇没什么好改的」比硬找问题更可信——假阳性控制的最终形态是敢于出全绿报告
   - **学生档案行协议**（跨会话个性化，skill 无状态故状态外置给用户）：批改报告的最末固定附一行「学生档案行」——紧凑格式如 `档案｜搭配:介词×2,动宾×1｜语域:口语化×1｜语法:时态×1`，供老师随学生作业保存；下次批改同一学生时用户把档案行贴回开头，则聚焦类优先选档案中的高频类型（个体轨迹特异性，Vyatkina, 2013；持续性错误优先，Kao et al., 2025），并在分布表区分「持续错误」与「本次新增」——新增错误类型是发展的新战场，持续错误是聚焦干预的主攻方向。无档案行时按本次分布处理，不追问。群体基线供解读参考（不覆盖个体档案）：中文学生群体错误分布约 misformation 五成、omission 两成、词汇与机械类一成半（Sasi & Lai, 2021），个体分布偏离群体基线本身就是值得注意的信号
...
- **末尾汇总升级为教师版三件**：①错误类型分布表（全面诊断结果，按学术三项专项 + 通用搭配/语域/语义韵分组计数，学生最常错的排最前）；②本次最值得讲的一个点（与聚焦类的选择一致，配一条最强证据，教师可直接当课堂例句用；连续多次批改时提醒一句：写作进步呈 W 型非线性轨迹，单次表现波动不等于退步，Hiver et al., 2024）；③给学生的下一次自查清单（按本次错误类型轮换 2-3 条可操作动作，如「交稿前把所有 results 后面的动词圈出来，逐个问证据是相关还是因果」），每条动作对应本次报告里的实例编号。**变异信号解读**：同一搭配在学生多次作业里时对时错不是退步，是假设检验活跃的表现（个体内变异是发展的功能引擎，Verspoor et al., 2021）；需要警惕的反而是稳定不变地错（僵化信号）——前者鼓励尝试，后者才升级干预；**已纠正错误的回潮同理**——显式纠正过的结构在学期末等认知负荷升高时重现，是发展的正常波动而非教学失效（recursive regressions，Zhang & Zhang, 2023; Zhan, 2015）；另有发展升级律供家长与学生预期管理：表层语法错误下降常伴随篇章层问题上升，是修辞复杂度扩张的代价而非退步（Moradi, 2026）。教学建议：多篇作业连续批改时，本次聚焦类优先选学生上一篇的高频错误类型（聚焦干预针对持续性错误，Kao et al., 2025）；课堂场景可先让学生两两互标疑点再自查修订，同伴互标先于语料查询可提升错误识别率（Kim & Emeliyanova, 2019）
