...
4. **选择反馈模式**（WCF 文献驱动）：
   ...
   - **学生档案行协议**（跨会话个性化，skill 无状态故状态外置给用户）：批改报告的最末固定附一行「学生档案行」——紧凑格式如 `档案｜搭配:介词×2,动宾×1｜语域:口语化×1｜语法:时态×1`，供老师随学生作业保存；下次批改同一学生时用户把档案行贴回开头，则聚焦类优先选档案中的高频类型（个体轨迹特异性，Vyatkina, 2013；持续性错误优先，Kao et al., 2025），并在分布表区分「持续错误」与「本次新增」——新增错误类型是发展的新战场，持续错误是聚焦干预的主攻方向。无档案行时按本次分布处理，不追问
