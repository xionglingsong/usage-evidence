...
<p align="center"><a href="promo/usage-evidence-v1-promo.mp4" title="点击播放 52 秒宣传片"><img src="promo/poster.png" alt="usage-evidence 宣传片封面，点击播放"></a></p>
...
| 能力 | 说明 |
|---|---|
| 10 类查询 | 搭配查证 / 词义 / 语域 / 历时趋势 / 译法 / 同义辨析 / 搭配发现 / 语义韵 / **学术语域** / **引言摘要语步** |
| 通用源 13 个 + 学术源 4 个 | 牛津、剑桥、朗文、Linguee、Google Ngram、Linggle 等全部实测；学术线 OpenAlex（2.5 亿文献短语计数+趋势+学科过滤）、PubMed、arXiv、Google site: 语域对比，每个源的访问方式和坑都实测记录在案 |
| 批改模式（四链巡逻） | 通用九类标记 + **学术四链**：结论动词强度（prove/suggest 错配）、引用动词态度（argues/claims）、结论表达、引言/摘要语步（CARS 三语步 + 摘要学科期望表） |
| 三种反馈模式 | **聚焦**（默认，只深纠 1-2 类，认知负荷控制）/ 全面 / **确认**（全绿文本出人话点评，不硬找错） |
| 学生档案行 | 报告末尾一行紧凑档案，下次贴回即个性化批改——持续错误优先、个体轨迹追踪 |
| 证据形态纪律 | 频率数字、词典释义原话、例句原文、Ngram 曲线各就其位；词义歧义必须词典原话实锤；**频率三档制**——10 倍以上才下硬结论，3 倍内说并存 |
| 被拒指纹 | 方法/结果句时态语态转换失败是被拒摘要的统计指纹，按学科惯例级标记 |
| 回归防线 | scripts/smoke-test.mjs 五项不变量自测（源退化检测、PubMed 三态判读、版本链路），改版后跑一遍防回归 |
| 教师版三件套 | 错误类型分布表 + 最值得讲的一个点（可直接当课堂例句）+ 学生自查清单；变异信号解读（时对时错是假设检验活跃，稳定地错才警惕僵化） |
...
- 结论动词的强度阶梯：the results indicate 647 万篇、suggest 483 万篇，prove 只有 27 万篇（差 18-24 倍）——把相关性证据写成 prove 是审稿人一眼识破的高频错
- play a role **in** 是 on 的 245 倍（Ngram 图书语料定谳）；介词类查证走「OpenAlex 同值检测 → PubMed 验翻译 → Ngram 兜底」三路链路，因为实测发现 OpenAlex 对功能词组合会静默退化
- the East 在剑桥词典只有两个义项（Asia / 前共产主义东欧），「中国东部」不存在——词义歧义用词典原话实锤，不用 AI 语感判断
- 摘要学科期望表：工程摘要方法语步占 79-81% 篇幅且结论可省，社科摘要背景是必要语境——一把尺两套标准，不拿五语步模板卡所有人（500 篇里只有 2.4% 用满五语步）

## 输出样例

两种模式的成品输出，全部证据可点击回溯：

- **单点查证（速览版，学生日常）**：[examples/quick-check-sample.md](examples/quick-check-sample.md) —— 问一句「Several researches 对吗」，15 秒拿到词典原文证据
- **学术批改（教师版三件套）**：[examples/academic-proofread-sample.md](examples/academic-proofread-sample.md) —— 整段摘要批改，错误分布表 + 最值得讲的一个点 + 学生自查清单
...
这个 skill 管用词，不管语法——语料查询对语法准确性的提升不显著（Kızıl, 2023），语法请交给专门的检查工具。Ngram 是书籍语料，口语和 2019 后新词覆盖弱。学科惯例问题优先走学术源（OpenAlex 学科过滤 / PubMed / arXiv），通用语料与学科源冲突时以学科源为准（Flowerdew & Petrić, 2024）。查不到就如实说查不到，不硬下结论。批改设计有文献依据：聚焦反馈、带工具自查、时态指纹，全部锚点见 CHANGELOG 与 evidence/ 目录。
